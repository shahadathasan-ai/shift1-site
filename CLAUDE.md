...
- **Screenplay source is embedded** in `<script type="text/plain" id="fountain">` (a copy of `Writing /Shuffle/Shuffle v3.fountain`) so the page renders offline without a fetch. `read/shuffle.fountain` is a standalone copy of the same source.
...
- **Storyboard frames** (`FRAMES` array): 11 frames, each anchored either to a scene-heading substring (`key`) or an in-scene action phrase (`at`), first unused match wins, placed beside — and vertically aligned to — the block it matches. Beat-anchored frames (via `at`): **06 The Stopped Foot** (`at:'HOLDS IT MID-AIR'`), **07 The Sink** (`at:'THE WATER HITS'`, the black-water beat), **08 The Corner** (`at:'SUDDENLY A NOISE'`), **09 The Scale** (`at:'FOLDED WRONG'`, the dead-Pike reveal), and **11 The Scream** (`at:'IT SCREAMS'`). An optional `skip:N` jumps past the first N matching blocks — used by **10 The Reveal** (the climax figure is the *second* `SUPPLY ROOM - CONTINUOUS`). Frames with a real render show the image; the rest show a labelled dark placeholder ("render coming"). **All 11 frames now have real Midjourney stills** in `read/frames/` (sourced from `Writing /Shuffle/Pictures/`): **01 The Plaza** (`plaza.jpg`), **02 The Descent** (`elevator-interior.jpg`), **03 The Locker Room** (`changing-room.jpg`), **04 The Corridor** (`corridor.jpg`), **05 The Fume Hood** (`fume-hood.jpg`), **06 The Stopped Foot** (`stopped-foot.jpg`), **07 The Sink** (`sink.jpg` — hands under running water, black liquid swirling in the porcelain), **08 The Corner** (`monster-darkness.jpg` — the figure in the dark lab on his return), **09 The Scale** (`pike-in-bin.jpg` — the body-in-bin reveal on the scale), **10 The Reveal** (`supply-room.jpg`), and **11 The Scream** (`monster.jpg` — a charcoal-drawn screaming figure; earlier options `Scream.png`/`Monster 2.png` kept in `Pictures/` as alternates). No placeholders remain. **All frames share the same 2.33:1 letterbox** — Frame 11 (`monster.jpg`, a charcoal illustration) was cropped from its original 1024×1024 square to 1024×439 (top kept, lower body dropped) to match the others. The renderer still supports a per-frame `ratio` override, but it's currently unused. *(Note: frame numbers 03/04 read in reverse scene order — the Corridor scene precedes the Locker Room in the script — left as-is pending a call.)*
...
│   ├── shuffle.fountain    ← screenplay source copy (from Writing /Shuffle/Shuffle v3.fountain)
│   ├── frames/             ← storyboard stills (all 11: plaza, elevator-interior, changing-room, corridor, fume-hood, stopped-foot, sink, monster-darkness, pike-in-bin, supply-room, monster)
│   └── pdfs/               ← 6 story PDFs
├── Writing /
